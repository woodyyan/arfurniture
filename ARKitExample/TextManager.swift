	...
	func escalateFeedback(for trackingState: ARCamera.TrackingState, inSeconds seconds: TimeInterval) {
		...
		self.trackingStateFeedbackEscalationTimer = Timer.scheduledTimer(withTimeInterval: seconds, repeats: false, block: { _ in
			...
			case .notAvailable:
				...
				title = "追踪状态：不可用。"
                // Tracking status has been unavailable for an extended time. Try resetting the session.
				message = "追踪状态在很长一段时间内都是不可用的。尝试重置会话。"
			case .limited(let reason):
                // Tracking status: Limited.
				title = "追踪状态：受限。"
                // Tracking status has been limited for an extended time.
				message = "追踪状态在很长一段时间内都被限制了。"
                //Try slowing down your movement, or reset the session.
                //Try pointing at a flat surface, or reset the session.
				switch reason {
				case .excessiveMotion: message += "试着放慢你的动作，或者重置会话。"
				case .insufficientFeatures: message += "试着指向一个平面，或者重新设置会话。"
				}
			case .normal: break
			}
			
			let restartAction = UIAlertAction(title: "重置", style: .destructive, handler: { _ in
